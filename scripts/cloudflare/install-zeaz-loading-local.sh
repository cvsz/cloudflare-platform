...
#   bash scripts/cloudflare/install-zeaz-loading-local.sh --brand "ZeaZ" --app-url "https://app.zeaz.dev"
...
Environment overrides:
  BRAND_NAME, APP_URL, ROUTE_PATTERN, ZONE_NAME, STATUS_TEXT_TH, STATUS_TEXT_EN
USAGE
}
...
ensure_node_tooling() {
  ...
  if ! command -v npx >/dev/null 2>&1; then
    fail "npx not found. Install a recent Node.js/npm first."
  fi
}
...
run_preview() {
  ...
  cd "${ROOT_DIR}"
  npx wrangler dev --config "${WRANGLER_CONFIG}" --local --ip 127.0.0.1 --port 8787
}
